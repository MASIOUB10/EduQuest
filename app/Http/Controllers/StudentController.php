...
class StudentController extends Controller
{
    ...
    {
        $user = Auth::user();
        $validCourses = CourseRequest::where('student_id', $user->id)->where('status', 'valide')->with('course')->get();
        $notValidCourses = CourseRequest::where('student_id', $user->id)->where('status', 'notvalide')->with('course')->get();
