...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="{{ route('admin.courses.update', $course->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $course->description) }}</textarea>
        </div>
        <div>
        ...
        <div>
            ...
            <input type="text" name="image" id="image" value="{{ old('image', $course->image) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        <div>
